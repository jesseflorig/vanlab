...
- [X] T017 [P] [US2] Add `gitea.oauth` and `gitea.config.oauth2_client` sections to `roles/gitea/templates/values.yaml.j2` under the `gitea:` key — add `oauth:` list with one entry: `name: authentik`, `provider: openidConnect`, `key: "{{ gitea_oidc_client_id }}"`, `secret: "{{ gitea_oidc_client_secret }}"`, `autoDiscoverUrl: https://authentik.fleet1.lan/application/o/gitea/.well-known/openid-configuration`, `scopes: "openid profile email groups"`, `groupClaimName: groups`, `adminGroup: admins`; and under `config:` add `oauth2_client:` with `ENABLE_AUTO_REGISTRATION: true`, `USERNAME: preferred_username`, `UPDATE_AVATAR: true`, `ACCOUNT_LINKING: login`
- [ ] T018 [US2] Re-run Gitea Ansible role: `ansible-playbook -i hosts.ini playbooks/cluster/services-deploy.yml --tags gitea -e "ansible_ssh_common_args='-o StrictHostKeyChecking=no'"` — wait for Gitea pod rollout
- [ ] T019 [US2] Verify Gitea OIDC login: navigate to `https://gitea.fleet1.lan` → **Sign In** → click **authentik** button → complete OIDC flow as `akadmin` → confirm site admin badge is visible in the user menu
